...
import 'package:shopnew/services/EmailService.dart'; // Đảm bảo import đúng file này
...
class _ShoppingState extends State<Shopping> {
  ...
  double tong = 0;
  String? email, name; // 1. Đã thêm biến name
  ...
  getthesharedpref() async {
    ...
    name = await Share_pref().getUserName(); // 2. Lấy tên người dùng
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              SizedBox(height: 8),
              Text(displayAddress, style: TextStyle(fontSize: 15, color: Colors.black87)),
            ],
          ),
        );
      },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: Color(0xfff2f2f2),
        title: Text("Giỏ hàng", style: Appwidget.semiboldTextStyle()),
  ...
  displayPaymentSheet(orderInfoMap) async {
    try {
      ...
      // 3. LOGIC GỬI MAIL ĐÃ ĐƯỢC SỬA CHO GIỎ HÀNG
      // Tạo chuỗi danh sách sản phẩm (Ví dụ: iPhone 13 (x2), Samsung S24 (x1))
      String productListString = cartItems.map((e) => "${e["Product"]} (x${e["Count"]})").join(", ");

      await EmailService.sendOrderConfirmation(
        userEmail: email!,
        userName: name!,
        productName: productListString, // Gửi danh sách tên sản phẩm
        price: "${currencyFormat.format(tong)}₫", // Gửi tổng tiền
